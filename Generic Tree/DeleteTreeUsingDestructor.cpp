...
//preorer traversal :first print parent node then children
...
//postorder traversal : first children then node
...
//first delete children then delete root otherwise address of children will be lost and children will not be deleted
...
int main(){
    ...
    cout<<endl;
    // deleteTree(root);
    // printTree(root);  tree will not  be printed because tree is deleted
    delete root;
    //each time you call delete root first code code inside destructor will be executed then node will be deleted
    return 0;
}